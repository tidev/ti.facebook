exports.window = function () {
	...
	fb.addEventListener('login', function (e) {
		...
		if (e.success) {
			showAlert('login from uid: ' + e.uid + ', name: ' + JSON.parse(e.data).name);
	...
	var customLoginButton = Ti.UI.createButton({
		title: 'Custom Log in',
		top: 50,
		width: 160,
		height: 40
	});
	win.add(customLoginButton);
	...
	function updatePublishPerms() {
		if (doPublish.value && fb.loggedIn) {
			fb.requestNewPublishPermissions([ 'publish_actions' ], fb.AUDIENCE_FRIENDS, function (e) {
				if (e.success) {
					showAlert('request publish permission success');
				} else if (e.cancelled) {
					showAlert('user cancelled');
				} else {
